// Largely stolen from Philip McLauchlan's Horatio Libraries
// Functions for creating Ximages
// The important function is hor_convert_image_to_X_format at the end
// of the file.
			   ...
			   int pixel_size_factor )
{
   ...
   cwidth = (width-1)*hor_subsample_c/pixel_size_factor + 1;
   cheight = (height-1)*hor_subsample_r/pixel_size_factor + 1;
   xdata = hor_malloc ( sizeof(short) * cwidth*cheight );
   ptr = (u_short *) xdata;
...
static int *int_array_pixel_factor_1 ( Hor_Imdata     imarray,
					Hor_Image_Type type,
					int        width,
					int        height )
{
   ...
   xdata = hor_malloc ( sizeof(int)*size );
   ptr = (u_int *) xdata;
   switch ( type )
   {
      case HOR_BIT:
	 ...
	 for ( aend = imarray.s[0] + width*height, arptr = imarray.s[0];
      ...
      default:
      hor_error ( "invalid image type (int_array_pixel_factor_1)", HOR_FATAL);
      break;
   }
...
/* int_array_pixel_factor_2(): copies each internal format pixel into area of
                                 display memory 2*2 canvas pixels */

static int *int_array_pixel_factor_2 ( Hor_Imdata     imarray,
					 Hor_Image_Type type,
					 int            width,
					 int            height )
{
   ...
   switch ( type )
   {
	 ...
	 for ( row = 0; row < height; row++ )
	 {
      ...
      default:
      hor_error ( "invalid image type (int_array_pixel_factor_2)", HOR_FATAL );
      break;
   }
...
/* int_array_any_pixel_factor(): copies each internal format pixel into area
                                   of display memory
		 		   pixel_size_factor*pixel_size_factor canvas
				   pixels */
static int *int_array_any_pixel_factor ( Hor_Imdata     imarray,
					   Hor_Image_Type type,
					   int width, int height,
					   int pixel_size_factor_c,
					   int pixel_size_factor_r )
{
   ...
   int       size = width*height*pixel_size_factor_c*pixel_size_factor_r;
   int       row, count1, count2;
   u_int **ptrarray, *ptr, colour;

   xdata = hor_malloc ( sizeof(int) * size );
   ptrarray = hor_malloc_ntype ( u_int *, pixel_size_factor_r );
   ptrarray[pixel_size_factor_r-1] = (u_int *) xdata;
   switch ( type )
   {
      case HOR_BIT:
      {
	 ...
	 for ( row = 0; row < height; row++ )
	 {
      ...
      default:
      hor_error ( "invalid image type (int_array_any_factor)", HOR_FATAL );
      break;
   }
...
/* int_array(): converts image data from internal format to X display format*/
static int *int_array ( Hor_Imdata     imarray,
			   Hor_Image_Type type,
			   int width,           int height,
			   int hor_subsample_c, int hor_subsample_r,
			   int pixel_size_factor )
{
   ...
   if ( pixel_size_factor > 0 )
   {
      ...
      if ( pixel_size_factor_c == 1 && pixel_size_factor_r == 1 )
      /* usually HOR_TRUE, so make this run fast */
      return ( int_array_pixel_factor_1 ( imarray, type, width, height ) );
      else if ( pixel_size_factor_c == 2 && pixel_size_factor_r == 2 )
	 /* this is also quite common, e.g. displaying 256*256 images
	    on a 512*512 canvas, so make this run fast too */
	 return ( int_array_pixel_factor_2 ( imarray, type, width, height ) );
      else /* the general case */
	 return ( int_array_any_pixel_factor ( imarray, type, width, height,
					        pixel_size_factor_c,
					        pixel_size_factor_r ) );
   }
   ...
   cwidth = (width-1)*hor_subsample_c/pixel_size_factor + 1;
   cheight = (height-1)*hor_subsample_r/pixel_size_factor + 1;
   xdata = hor_malloc ( sizeof(int) * cwidth*cheight );
   ptr = (u_int *) xdata;
...
/* int_array_float(): converts image data from internal float format
                       to X display format*/
static int *int_array_float ( Hor_Imdata imarray,
				 int    width,           int    height,
				 int    hor_subsample_c, int hor_subsample_r,
				 int    pixel_size_factor,
				 float  black_val, float  white_val )
...
/* int_array_int(): converts image data from internal int format
                      to X display format*/
static int *int_array_int ( Hor_Imdata imarray,
			       int width,           int height,
			       int hor_subsample_c, int hor_subsample_r,
			       int pixel_size_factor,
			       int black_val, int white_val )
				        ...
				        va_list   *aptr )
{
   ...
   if (pixel_size_factor > 0) {
       xim_width = imptr->width*hor_subsample_c*pixel_size_factor;
       xim_height = imptr->height*hor_subsample_r*pixel_size_factor;
   } else {
       xim_width = (imptr->width-1)*hor_subsample_c/(-pixel_size_factor) + 1;
       xim_height = (imptr->height-1)*hor_subsample_r/(-pixel_size_factor) + 1;
   }
   ...
   switch ( depth )
   {
      ...
      xim = XCreateImage (display, DefaultVisual (display, scrnum),
			  depth, ZPixmap, 0, NULL, xim_width, xim_height,
			  8, 0 );
      if (xim->bits_per_pixel != 8) {
	  hor_print("Display depth 8 does not equal XImage bits_per_pixel (%d)\n", 
		    xim->bits_per_pixel);
	  XFree(xim);
	  return NULL;
      }
	 ...
	 break;
	  ...
	  hor_print("Display depth 16 does not equal XImage bits_per_pixel (%d)\n", 
		    xim->bits_per_pixel);
	  XFree(xim);
	  return NULL;
      }
	 ...
	 break;
      ...
      {
         case HOR_INT:
	 xdata = (char *)int_array_int (imptr->array, imptr->width, imptr->height,
					hor_subsample_c, hor_subsample_r,
					pixel_size_factor,
					(int) (va_arg ( *aptr, double ) + 0.5),
					(int) (va_arg ( *aptr, double ) + 0.5));
	 break;
         ...
         case HOR_FLOAT:
	 xdata = (char *)int_array_float (imptr->array, imptr->width, imptr->height,
					  hor_subsample_c, hor_subsample_r,
					  pixel_size_factor,
					  va_arg ( *aptr, double ),
					  va_arg ( *aptr, double ) );
	 break;
         ...
         default:
	 xdata = (char *)int_array (imptr->array, imptr->type,
				    imptr->width, imptr->height,
				    hor_subsample_c, hor_subsample_r,
				    pixel_size_factor );
	 break;
