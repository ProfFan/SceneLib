...
Hor_Bool alloc_colour ( Display                 *display,
			       Colormap                 cmap,
			       long                    *top_grey_level_ptr,
			       Hor_X_to_Image_ColourMap x_to_image_colourmap,
			       u_long                  volatile *image_to_x_colourmap,
			       const char              *colour_name,
			       u_long                  volatile *colour_ptr )
{
   ...
   *colour_ptr = all_colours[no_colours++] = screen_col.pixel;
   ...
   switch ( hor_get_image_display_format() )
   {
      ...
      case HOR_U_SHORT:
      if (x_to_image_colourmap.us != NULL)
   ...
   top_grey_level:     highest available grey level in internal format */
{
   ...
   *grey_ptr = hor_malloc_ntype ( u_long, internal_grey_levels );
   ...
   if (cpu_byte_order == ImageByteOrder(display)) { /* no need to swap */
       ...
       switch DefaultDepth ( display, DefaultScreen(display) ) {
       case 8:
	   /* no need to swap anyway! */
	   for (level=0; level<internal_grey_levels; level++)
	       image_grey_values[level] = (*grey_ptr)[level];
	   break;
       case 16:
       ...
       case 32:
	   for (level=0; level<internal_grey_levels; level++)
	       image_grey_values[level] = ((((*grey_ptr)[level])&0xFF000000)>>24) |
		                          ((((*grey_ptr)[level])&0xFF0000)>>8) | 
					  ((((*grey_ptr)[level])&0xFF00)<<8) |
					  ((((*grey_ptr)[level])&0xFF)<<24);
	   break;
       default:
	   /* unsupported so don't swap */
	   hor_warning("unsupported byte swap required for this depth screen");
	   for (level=0; level<internal_grey_levels; level++)
	       image_grey_values[level] = (*grey_ptr)[level];
       }
   }
...
static void alloc_x_to_image ( int                       depth,
			       ...
			       u_long                  **image_to_x_colourmap )
{
   int x_colours = hor_power_of_two ( depth );

   switch ( hor_get_image_display_format() )
   {
      case HOR_U_CHAR:
      if (depth > 32) {
	  /* have to allocate to big an x_to_image_colourmap array */
	  x_to_image_colourmap->uc = NULL;
	  hor_print("Warning: bitplane depth (%d) to large for x_to_image_colourmap array\n", depth);
	  hor_print("         hor_display_write_to_file() disabled\n");
      } else {
