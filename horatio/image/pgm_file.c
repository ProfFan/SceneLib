/* Copyright 1994 Andrew Wildenberg (andrew@robots.oxford.ac.uk) and
...
*                                    Hor_Image *imptr, ... )
*
*   hor_read_pgm_image_stream() reads and returns an PGM image read from the
*   given file stream. hor_read_pgm_image() opens the given file and calls
*   hor_read_pgm_image_stream().
*
*   PGM format image file I/O functions.
*
*   hor_write_pgm_image_stream() outputs the given image to a file stream.
*   Extra arguments are required to convert images of type HOR_FLOAT to
*   an unsigned char file. hor_write_pgm_image() opens the given file and calls
*   hor_write_pgm_image_stream().
********************/
Hor_Image *hor_read_pgm_image_stream ( int fd )
{
   Hor_Image *result;
   char       buffer[100];
   int        width, height, maxval, ptype;
   FILE      *fp;
   ...
   result = hor_alloc_image ( width, height, (ptype==5) ? HOR_U_CHAR : HOR_RGB_UC, NULL );
   if ( result == NULL )
   {
   ...
   if (ptype==5) {
       fread ( result->array.uc[0], 1, width*height, fp );
   } else {
...
Hor_Bool hor_write_pgm_image_stream ( int fd, Hor_Image *imptr, ... )
{
   ...
   fwrite ( imptr->array.uc[0], 1, imptr->width*imptr->height, fp );
   fclose ( fp );
   return HOR_TRUE;
}
...
Hor_Bool hor_write_pgm_image ( const char *file_name, Hor_Image *imptr, ... )
{
   int      fd = -1;
   Hor_Bool result;
   ...
   result = hor_write_pgm_image_stream ( fd, imptr );
